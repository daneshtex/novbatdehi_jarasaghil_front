...
export default function ViewEditCarPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!validateForm()) return; // بررسی اعتبارسنجی

    updateCarMutation.mutate(formData, {
      onSuccess: () => {
        toast.success("تغییرات با موفقیت ذخیره شد!"); // ✅ نمایش toast
        navigate("/dashboard/cars"); // ✅ بازگشت به صفحه لیست
      },
      onError: (err) => {
        console.error(err);
        // خطاها قبلاً در mutation مدیریت شده
      },
    });
  };
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
            {isEditing ? "ویرایش انواع کانتینر  " : "مشاهده انواع کانتینر"}
          </h1>
          <p className="text-gray-600 mt-2 text-sm sm:text-base">
            {isEditing
              ? "اطلاعات انواع کانتینر را ویرایش کنید"
              : "اطلاعات کامل انواع کانتینر"}
          </p>
        </div>
